...
const BodyContainer = (props) => {
  ...
  for(let i = 0; i < props.movieList.length; i++) {
    if(props.movieList[i].fav === true) {
      fetch(`https://api.themoviedb.org/3/movie/${props.movieList[i].TMDBid}?api_key=b77e6bcb363054a49df8e588ecd2fdc6`)
      .then((res) => res.json())
      .then(movieData => {
        ...
        fetch(`https://api.themoviedb.org/3/discover/movie?api_key=b77e6bcb363054a49df8e588ecd2fdc6&language=en-US&sort_by=popularity.desc&with_genres=${max}`)
        .then((res) => res.json())
      .then(movieData => {
  ...
  return (
    ...
    <div className="body-container">
      ...
      <div className="card-container">
        {searchBarMovies.length !== 0 ? searchBarMovies.map((movie, i) => <SearchBarMedia key={`searchMovie${i}`} tmdbId={movie.id} />) : null}
      </div>
